...
# or PM₂.₅ concentration) for that city. It is used as a helper by many of the
# scripts used in generating Figures 1, 2, and S5.
def calc_city_pwm(target_city, all_cities, datapoint, pop):
   condition = ((all_cities == target_city) &
                (~np.isnan(datapoint)) & (~np.isnan(pop)))
   datapoint = datapoint[condition]
   pop = pop[condition]
   return np.average(datapoint, weights = pop)

# `collect_data` creates and returns a list of lists that contain NO₂ levels
# (measured in ppb) and PM₂.₅ levels (measured in (μg/m³)), organized by both
# HOLC grade (A, B, C, or D), as well as the demographics of residents living
# in any given block. Out of all the plots generated by the study, this is the
# most simply organized (i.e., involves the last number of additional
# computations), and is the first one seen, shown in Figure 1.
def collect_data(df, population_factor, target_data, calc_diff):
   ...
   for i in df.index:
      if df['Grade'][i] == 'A':
         HOLC_A_data += (round(df[population_factor][i] * df['Total'][i]) *
                         [(df[target_data][i]) - eval(diff)])
      if df['Grade'][i] == 'B':
         HOLC_B_data += (round(df[population_factor][i] * df['Total'][i]) *
                         [(df[target_data][i]) - eval(diff)])
      if df['Grade'][i] == 'C':
         HOLC_C_data += (round(df[population_factor][i] * df['Total'][i]) *
                         [(df[target_data][i]) - eval(diff)])
      if df['Grade'][i] == 'D':
         HOLC_D_data += (round(df[population_factor][i] * df['Total'][i]) *
                         [(df[target_data][i]) - eval(diff)])
      if df['Grade'][i] != 'N':
         white += (round(df[population_factor][i] * df['White'][i]) *
                   [(df[target_data][i]) - eval(diff)])
         other += (round(df[population_factor][i] * df['Other'][i]) *
                   [(df[target_data][i]) - eval(diff)])
         black += (round(df[population_factor][i] * df['Black'][i]) *
                   [(df[target_data][i]) - eval(diff)])
         asian += (round(df[population_factor][i] * df['Asian'][i]) *
                   [(df[target_data][i]) - eval(diff)])
         hispanic += (round(df[population_factor][i] * df['Hispanic'][i]) *
                      [(df[target_data][i]) - eval(diff)])
   
   return [HOLC_A_data, HOLC_B_data, HOLC_C_data, HOLC_D_data,
...
def collect_data_intraurban_diff(df, population_factor, target_data):
   ...
   def collect_demographic_data(df, population_factor, population, target_data, pwms):
      ...
      for i in df.index:
         pwm = pwms[df['City'][i]]
         if df['Grade'][i] == 'A':
            A_data += (round(df[population_factor][i] * df[population][i]) *
                       [df[target_data][i] - pwm])
         if df['Grade'][i] == 'B':
            B_data += (round(df[population_factor][i] * df[population][i]) *
                       [df[target_data][i] - pwm])
         if df['Grade'][i] == 'C':
            C_data += (round(df[population_factor][i] * df[population][i]) *
                       [df[target_data][i] - pwm])
         if df['Grade'][i] == 'D':
            D_data += (round(df[population_factor][i] * df[population][i]) *
                       [df[target_data][i] - pwm])
      return [np.average(A_data), np.average(B_data),
              np.average(C_data), np.average(D_data)]
...
def collect_data_percentile(df, population_factor, population, target_data):
   ...
   for i in df.index:
      cumulative += (round(df[population_factor][i] * df[population][i]) * [df[target_data][i]])
      if df['Grade'][i] == 'A':
         percentile_A += (round(df[population_factor][i] * df[population][i]) * [df[target_data][i]])
      if df['Grade'][i] == 'B':
         percentile_B += (round(df[population_factor][i] * df[population][i]) * [df[target_data][i]])
      if df['Grade'][i] == 'C':
         percentile_C += (round(df[population_factor][i] * df[population][i]) * [df[target_data][i]])
      if df['Grade'][i] == 'D':
         percentile_D += (round(df[population_factor][i] * df[population][i]) * [df[target_data][i]])

   return (np.percentile(percentile_A, percentiles),
           np.percentile(percentile_B, percentiles),
           np.percentile(percentile_C, percentiles),
           np.percentile(percentile_D, percentiles),
...
def collect_data_city(df, population_factor, population, target_data, calc_diff, calc_per):
   ...
   diff = "0"
   for city in cities:
      city_pops[city] = calc_city_pop(city, df['City'], df['Grade'], df[population_factor], df[population])
   if calc_diff:
      pwms = {}
      for city in cities:
   ...
   for i in df.index:
      if df['City'][i] != 'NA':
         datapoint = (round(df[population_factor][i] * df[population][i]) * [((df[target_data][i]) - eval(diff)) / eval(per)])
         collect_city_measurements(df['City'][i], df['Grade'][i], small_cities, small_city_data, datapoint)
         collect_city_measurements(df['City'][i], df['Grade'][i], medium_cities, medium_city_data, datapoint)
         collect_city_measurements(df['City'][i], df['Grade'][i], large_cities, large_city_data, datapoint)
